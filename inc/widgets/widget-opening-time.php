...
if ( ! class_exists( 'PW_New_Opening_Time' ) ) {
	class PW_New_Opening_Time extends WP_Widget {
		/**
		 * @var mixed The ID of the current widget.
		 */
		private $current_widget_id;

		/**
		 * PW_New_Opening_Time constructor.
		 */
		public function __construct() {
			parent::__construct(
				'pw_opening_time',
				sprintf( 'ProteusThemes: %s', esc_html__( 'Opening Time', 'woondershop-pt' ) ),
				array(
					'description' => esc_html__( 'Displays a table of your opening hours.', 'woondershop-pt' ),
					'classname'   => 'widget-opening-time',
				)
			);
		}
		...
		?>
			...
			<?php foreach ( $items as $item ) : ?>
				<div class="opening-time__item<?php echo ! empty( $item['less-important'] ) ? '  opening-time__item--less-important' : ''; ?>">
					...
					<div class="opening-time__separator"></div>
		...
		?>
			...
			<h4><?php esc_html_e( 'Opening time rows', 'woondershop-pt' ); ?></h4>
			...
			<script type="text/template" id="js-pt-opening-time-item-<?php echo esc_attr( $this->current_widget_id ); ?>">
				<div class="pt-sortable-setting  ui-widget  ui-widget-content  ui-helper-clearfix  ui-corner-all">
					<div class="pt-sortable-setting__header  ui-widget-header  ui-corner-all">
						...
						<span><?php esc_html_e( 'Opening time row', 'woondershop-pt' ); ?> - </span>
					...
					<div class="pt-sortable-setting__content">
						...
						<p>
							<label for="<?php echo esc_attr( $this->get_field_id( 'items' ) ); ?>-{{id}}-day"><?php esc_html_e( 'Day:', 'woondershop-pt' ); ?></label>
						...
						<p>
							<label for="<?php echo esc_attr( $this->get_field_id( 'items' ) ); ?>-{{id}}-time"><?php esc_html_e( 'Time:', 'woondershop-pt' ); ?></label>
						...
						<p>
							...
							<label for="<?php echo esc_attr( $this->get_field_id( 'items' ) ); ?>-{{id}}-less-important"><?php esc_html_e( 'Is this item less important?', 'woondershop-pt' ); ?></label>
							<br>
							<small><?php esc_html_e( 'Useful for lunch breaks or other less important notifications.', 'woondershop-pt' ); ?></small>
						</p>

						<p>
							<input name="<?php echo esc_attr( $this->get_field_name( 'items' ) ); ?>[{{id}}][id]" class="js-pt-opening-time-id" type="hidden" value="{{id}}" />
							<a href="#" class="pt-remove-opening-time-item  js-pt-remove-opening-time-item"><span class="dashicons dashicons-dismiss"></span> <?php esc_html_e( 'Remove item', 'woondershop-pt' ); ?></a>
						</p>
			...
			<div class="pt-widget-opening-time-items" id="opening-time-items-<?php echo esc_attr( $this->current_widget_id ); ?>">
				...
				<p>
					<a href="#" class="button  js-pt-add-opening-time-item"><?php esc_html_e( 'Add new item', 'woondershop-pt' ); ?></a>
				</p>
			</div>
			...
			<script type="text/javascript">
				(function( $ ) {
					...
					if ( _.isFunction( WoonderShop.Utils.repopulateOpeningTimeItems ) ) {
						WoonderShop.Utils.repopulateOpeningTimeItems( itemsJSON, widgetId );
					}
