	...
	public function update_notice() {
		...
		if ( isset( $api_response['is_site_active'] ) && false == $api_response['is_site_active'] ) {
			...
			<div class="notice notice-error is-dismissible">
				...
				<?php
				printf(
					esc_html__( 'Theme update is available: %1$s %2$s! %5$sHowever, your theme is not registered correctly and the theme update will fail because of that! Please %7$sregister the theme%4$s again with the same license key in order to solve the update issue%6$s. %3$sRead more about this issue here%4$s.', 'woondershop-pt' ),
		...
		$update_notice_string = esc_html__( "Updating this theme will lose any customizations you have made to the original theme files (customizer settings will not be lost). 'Cancel' to stop, 'OK' to update.", 'woondershop-pt' );
		...
		?>
			...
			<?php
				printf(
					esc_html__( 'Theme update is available: %5$s%1$s %2$s%6$s! %5$s%3$sUpdate now%4$s%6$s (check %7$s%1$s changelog%4$s).', 'woondershop-pt' ),
					esc_html( $this->theme['item_name'] ),
